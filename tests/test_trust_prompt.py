...
def test_a_tab_with_a_live_session_is_not_reported():
    assert main_mod._never_started(_plan([CWD_A]), _spawned(_plan([CWD_A])), {norm(CWD_A): 1}, elapsed=_LATE) == []


def test_a_tab_that_never_started_is_reported():
    assert main_mod._never_started(_plan([CWD_A]), _spawned(_plan([CWD_A])), {}, elapsed=_LATE) == [CWD_A]


def test_every_silent_tab_is_named():
    assert main_mod._never_started(_plan([CWD_A, CWD_B]), _spawned(_plan([CWD_A, CWD_B])), {}, elapsed=_LATE) == [CWD_A, CWD_B]
...
def test_a_claude_tab_that_never_started_is_reported_too():
    ...
    assert main_mod._never_started(plan, _spawned(plan), {}, elapsed=_LATE) == [CWD_A]


def test_an_empty_plan_reports_nothing():
    assert main_mod._never_started([], _spawned([]), {}, elapsed=_LATE) == []


def test_a_partly_started_plan_names_only_the_silent_one():
    assert main_mod._never_started(_plan([CWD_A, CWD_B]), _spawned(_plan([CWD_A, CWD_B])), {norm(CWD_A): 1}, elapsed=_LATE) == [CWD_B]
...
def test_a_repo_whose_directory_is_missing_is_not_blamed_on_a_trust_prompt():
    ...
    exist."""
    assert main_mod._never_started(_plan(missing=[CWD_GONE]), _spawned(_plan(missing=[CWD_GONE])), {}, elapsed=_LATE) == []
...
def test_a_repo_that_was_already_running_is_not_reported():
    ...
    assert main_mod._never_started(_plan(skipped=[CWD_A]), _spawned(_plan(skipped=[CWD_A])), {}, elapsed=_LATE) == []


def test_only_the_launched_tab_is_named_among_all_three():
    plan = _plan(launched=[CWD_B], skipped=[CWD_A], missing=[CWD_GONE])

    assert main_mod._never_started(plan, _spawned(plan), {}, elapsed=_LATE) == [CWD_B]
